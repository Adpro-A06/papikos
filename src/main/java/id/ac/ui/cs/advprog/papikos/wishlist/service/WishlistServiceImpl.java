...
public class WishlistServiceImpl implements WishlistService {
    ...
    public List<Kos> getUserWishlistItems(UUID userId) {
        log.debug("Getting wishlist items for user {}", userId);
        ...
        try {
            ...
            if (userWishlist != null && userWishlist.getKosList() != null) {
                ...
                log.debug("Found {} kos items in wishlist for user {}", kosItems.size(), userId);
                return kosItems;
            }
            ...
            log.error("Error getting wishlist items for user {}: {}", userId, e.getMessage());
            return new ArrayList<>();
        }
    }
    ...
    private Wishlist getUserWishlistEntity(UUID userId) {
        return wishlistStorage.stream()
                .filter(wishlist -> wishlist.getUserId().equals(userId.toString()))
                .findFirst()
                .orElse(null);
    }
